...
function crearFormulario() {
  var ref = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Referencias');
  var range = ref.getRange('PAIS');
  var pais = range.getValue();

  if (pais == 'EEUU') {
    /************************************************************************************
     *                   Plantilla de formulario de DScF                                *
     * https://docs.google.com/forms/d/1MFULPCjIzdZH0z1689Pw2kIpjeThfKSzxmkZ0_1RjtA/edit *
     *************************************************************************************/

    var formulario = DriveApp.getFileById('1MFULPCjIzdZH0z1689Pw2kIpjeThfKSzxmkZ0_1RjtA').makeCopy(
      'Cuestionario Alumnos Data Scientist Fundamentals',
    ...
    Logger.log(
      'generarCopiarTest] para ' +
        '' +
        ' con id ' +
        excelRespuestas.getId() +
        ' es destino del formulario ' +
        formulario_drive.getName()
    );
  } else {
    /*************************************************************************************
     *                   Plantilla de formulario de DScF                                 *
     * https://docs.google.com/forms/d/1oAZGu693XBRq-LTG4xAZICLy11JEuLwv5e8jHvXh3YU/edit *
     *************************************************************************************/

    var formulario = DriveApp.getFileById('1oAZGu693XBRq-LTG4xAZICLy11JEuLwv5e8jHvXh3YU').makeCopy(
      'Cuestionario Alumnos Data Scientist Fundamentals',
      DriveApp.getRootFolder()
    );
    var form = FormApp.openById(formulario.getId());
...
function recopilarRespuestasFeedback() {
  ...
  if (excel_response) {
    ...
    if (excel_response_ss.getLastRow() - 1 > 0) {
      sheet
        .getRange(2, 3, excel_response_ss.getLastRow(), excel_response_ss.getLastColumn())
        .setValues(
          excel_response_ss
            .getActiveSheet()
            .getRange(2, 1, excel_response_ss.getLastRow(), excel_response_ss.getLastColumn())
            .getValues()
        );
    }
  }
