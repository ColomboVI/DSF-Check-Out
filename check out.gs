...
function mandarFormularioCheckOut() {
  ...
  if (test) {
    ...
    if (response == ui.Button.YES) {
      ...
      Logger.log('El usuario ha cerro el cuadro de dialogo');
      return;
    }
  }
}
...
function getEmails() {
  ...
  for (var i = 0; i < max_registros && table_emails[i][1]; i++)
    if (table_emails[i][2] != 'Baja') emails.push(table_emails[i][1].trim());
  return emails;
}
...
function recopilarRespuestasCheckOut() {
  ...
  if (excel_response) {
    ...
    if (excel_response_ss.getLastRow() - 1 > 0) {
      sheet
        .getRange(3, 1, excel_response_ss.getLastRow(), excel_response_ss.getLastColumn())
        .setValues(
          excel_response_ss
            .getActiveSheet()
            .getRange(1, 1, excel_response_ss.getLastRow(), excel_response_ss.getLastColumn())
            .getValues()
        );
    }
  }
